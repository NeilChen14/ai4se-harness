...
- [ ] **SPEC_PROCESS.md**：按 §4.4 记录 brainstorming 关键节点 + 至少 3 轮迭代节选 + AI 建议采纳/否决记录 + 技能反思。
- [ ] **§4.5 冷启动验证**：用一个**不同类型的新 agent**、仅凭 `SPEC.md` + `PLAN.md` 实现 1–2 个 task，不提供对话历史；要求"不确定即暂停询问"。把停顿点、spec 缺陷、解读分歧、以及据此对 SPEC/PLAN 的修订（关键 diff）写入 `SPEC_PROCESS.md`。
- [ ] 冷启动发现的问题回写本 PLAN（勾选/修正后才开始 T1）。
